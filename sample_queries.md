...
## 5. Joins with GROUP BY and ORDER BY (Segment Reconstruction Test)

### Oracle Source
```sql
SELECT B.BANK_NAME, C.COMMON_DETAIL_CODE_NAME, SUM(A.TRANS_AMT) AS TOTAL_AMT 
FROM EGAS.CMS_TRANS_LIST_T A 
   , EGAS.BANK_ACCT_T B 
   , EGAS.COMMON_DETAIL_CODE_T C 
WHERE A.BANK_ACCT_SEQ = B.BANK_ACCT_SEQ 
AND B.BANK_CODE = C.COMMON_DETAIL_CODE 
AND C.COMMON_CODE = '0021' 
...
```sql
 ...
 AND A.TRANS_DATE >= :START_DATE
 JOIN EGAS.COMMON_DETAIL_CODE_T C
  ON B.BANK_CODE = C.COMMON_DETAIL_CODE
 AND C.COMMON_CODE = '0021'
 WHERE 1 = 1 
GROUP BY B.BANK_NAME, C.COMMON_DETAIL_CODE_NAME 
 ORDER BY TOTAL_AMT DESC 
